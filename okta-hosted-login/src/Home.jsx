...
const Home = () => {
  ...
  const apimHandler = async () => {
    try {
      const { data } = await axios.get(
        "https://oktapoc-azr-shsr-cus-u-fde-ebejd3dgepe4hwf4.a01.azurefd.net/api/demo",
        {
          headers: {
      ...
      serResponse(data);
      console.log("APIM response: ", data);
    } catch (err) {
      console.log(err);
      serResponse(err?.message);
    }
  };
  ...
  return (
    <div id="home">
      <div>
        ...
        <button className="apim-button" onClick={apimHandler}>
          CLICK ME FOR APIM TESTING
        </button>
